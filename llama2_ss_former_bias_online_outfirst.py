...
# from utils.predictor import SSCNNPredictor_rnafm2
from utils.tools import equal5, load_data
...
def get_posbias(seqs, max_len, scale):
    ...
    # scaled_pair_scores = {pair: score * scale for pair, score in pair_scores.items()}
...
def collate_fn(batch, scale):
    ...
    # print(data_dict['pos_bias'].shape)
    # print(max_len)
...
def main(args) :
    ...
    #     f'{args.config_dir}/t5_config_{args.model_scale}.json' # args.model.name,
    # ) #! change
    model_config = get_model_args(args.model_scale, 'encoder', tokenizer.vocab_size)
    # model = SSCNNPredictor_rnafm2(extractor, model_config, is_freeze=args.is_freeze)
    # print(args.config)
    ...
    # lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, 32*per_steps_one_epoch)
    ...
    for epoch in range(args.restart, args.num_epochs):
        if args.restart != 0:
            checkpoint = torch.load('./ckpt/'+name+'.pth')
        ...
        if epoch % 5 == 4:
            ...
            torch.save(checkpoint, './ckpt/'+name+'.pth')
            ...
            if len(val_dataset)!=0:
                with torch.no_grad():
                    ...
                    for data_dict in val_loader:
                        ...
                        # del loss, logits, labels, bs, loss_list
                        # for key in data_dict:
                        #     data_dict[key] = data_dict[key].cpu()  # 移回CPU
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_dir', default= data_path)
...
#accelerate launch  --num_processes=4  llama2_ss.py --model_scale s --mode bprna  --is_freeze True 
# accelerate launch  --num_processes=4 --main_process_port 1479 llama2_ss.py --model_scale lx --mode bprna  --is_freeze True 