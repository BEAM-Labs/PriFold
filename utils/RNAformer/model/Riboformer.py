...
class Attention2d(nn.Module):
    ...
    def forward(self, pair_act, attention_mask, bias=None):
        ...
        weighted_avg = torch.matmul(attn_weights, value).permute(0, 1, 3, 2, 4) # (bs, seq, head, res, value_dim) -> (bs, seq, res, head, value_dim)
...
class RiboFormer(nn.Module):
    ...
    def __init__(self, config, extractor, is_freeze):
        ...
        self.positional_encoding = PositionalEncoding(config['model_dim'])
    ...
    def forward(self, data_dict, max_cycle=0):
        ...
        pair_latent = self.positional_encoding(pair_latent)
